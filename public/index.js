async function linkInputChanged(e) {
  ...
  let linkValue = encodeURIComponent(linkInput.value);

  if (linkValue.length > 0) {
    link.href = `${baseLink}?redirect=${linkValue}`;
    link.innerHTML = `${baseLink}?redirect=${linkValue}`;
  } else {
    link.href = baseLink;
...
async function uploadFile(file) {
  ...
  const response = await fetch("/upload", {
    ...
    .then((text) => {
      ...
      link.href = `https://${domain}/${text}`;
      link.innerText = `https://${domain}/${text}`;
      link.target = "_blank";
    ...
    .catch((err) => {
      ...
      errmsg.innerText =
        "An error occured while uploading your file. Please try again later.";
